...
Jetbot::Jetbot(ros::NodeHandle& nh) : nh_(nh) {
    init();
    ...
    non_realtime_loop_ = nh_.createTimer(update_freq, &Jetbot::update, this);
}
...
void Jetbot::update(const ros::TimerEvent& e) {
...
void Jetbot::write(ros::Duration elapsed_time) {
...
{
    ros::init(argc, argv, "jetbot_hardware_interface");
    ros::NodeHandle nh;
